...
void check_command(char ** command){
    clock_t starttime;
    clock_t endtime;
    clock_t current_wallclock_time;
...
void terminate(){
    ...
    for (int j=0;j<=process_no;j++){
        ...
        printf("Start time: %s", ctime(&st_time[j]));
        printf("Process id: %d\n",process_list[j][2]);
        //printf("End time: %s",ctime(&process_list[j][3]));
        duration = (double)(process_list[j][3] - process_list[j][1])*1000/CLOCKS_PER_SEC;
